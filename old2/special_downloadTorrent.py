...
while i < lense:
    ...
    if manga[3] == '':
        sqlstr = 'UPDATE manga SET state = 15 WHERE id = "%s"' % (manga[0])
        c.execute(sqlstr)
        conn.commit()
        i += 1
        continue
        ...
        if response.text == 'The torrent file could not be found. Most likely you have mistyped the URL, or the torrent is no longer available.':
            ...
            if errorflag2 >= 5:
                for mangatemp in errortemplist:
                    sqlstr = 'UPDATE manga SET state = 13 WHERE state = 15 AND id = "%s"' % (manga[0])
                    c.execute(sqlstr)
                conn.commit()
            ...
            qbt_client.torrents_add(torrent_files=response.content,
                                    save_path=config.qbit_torrent_path + idnum, category="specialehentai",
                                    rename=idnum)
            time.sleep(3)
            torrentinfo = ''
            ...
            if torrentinfo == '':
                ...
                sqlstr = 'UPDATE manga SET state = -2 WHERE id = "%s"' % (manga[0])
                c.execute(sqlstr)
                conn.commit()
            else:
                filename = torrentinfo.content_path[len(torrentinfo.save_path) + 1:]
                print(filename)
                sqlstr = 'UPDATE manga SET state = 14 ,filename = "%s" ,torrenthash = "%s" WHERE id = "%s"' % (
