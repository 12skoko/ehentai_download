        ...
        if torrent.category == 'specialehentai':
            ...
            if torrent['completion_on'] > 0:
                if os.path.isdir(
                    ...
                    sqlstr = 'UPDATE manga SET state = 7,remark="isdir" WHERE state = 14 and torrenthash = "%s"' % (
                        torrent.hash)
                else:
                    sqlstr = 'UPDATE manga SET state = 7 WHERE state = 14 and torrenthash = "%s"' % (torrent.hash)
                # print(torrent.name)
                c.execute(sqlstr)
                conn.commit()
            if torrent['tags'] == 'fatel':
                sqlstr = 'UPDATE manga SET state = 15 , filename=NULL WHERE state = 14 and torrenthash = "%s"' % (
                    torrent.hash)
