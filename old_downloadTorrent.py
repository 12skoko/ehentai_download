...
while i < lense:
    ...
    if manga[3] == '':
        sqlstr = 'UPDATE manga SET state = 6 WHERE id = "%s"' % (manga[0])
        c.execute(sqlstr)
        conn.commit()
        i += 1
        continue
    ...
    if torrentLink == '':
        ...
        if response.text == 'The torrent file could not be found. Most likely you have mistyped the URL, or the torrent is no longer available.':
            ...
            qbt_client.torrents_add(torrent_files=response.content,
                                    save_path=config.qbit_torrent_path + idnum, category="ehentai",
                                    rename=idnum)
            time.sleep(3)
            torrentinfo = ''
            ...
            if torrentinfo == '':
                ...
                print(filename)
                sqlstr = 'UPDATE manga SET state = 5 ,filename = "%s" ,torrenthash = "%s" WHERE id = "%s"' % (
